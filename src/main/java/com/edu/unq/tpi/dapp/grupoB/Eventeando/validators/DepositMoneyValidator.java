...
public class DepositMoneyValidator {
    ...
    public User validateUser(User user) {
        validateNullityOf(user, new DepositException(DEPOSIT_MONEY_IS_INVALID_WITHOUT_USER));

        return user;
    }

    public LocalDate validateDate(LocalDate date) {
        validateNullityOf(date, new DepositException(DEPOSIT_MONEY_IS_INVALID_WITHOUT_DATE));

        return date;
    }

    private void validateNullityOf(Object field, DepositException exception) { if (field == null) { throw exception; } }

    public double validateAmount(double amount) {
        if (amount < 0) { throw new DepositException(AMOUNT_HAS_NEGATIVE_VALUE); }
